...
class OpenProjectClient:
    ...
    def get_work_package(self, work_package_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Debug: แสดง response content ก่อน parse JSON
            logger.info(f"Response status: {response.status_code}")
            logger.info(f"Response content type: {response.headers.get('content-type', 'unknown')}")
            logger.info(f"Response content length: {len(response.text)}")
            if len(response.text) < 500:  # แสดงเฉพาะถ้าสั้น
            ...
            logger.error(f"Failed to parse JSON response for work package {work_package_id}: {e}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="OpenProject Work Package Data Fetcher")
    parser.add_argument("--quiet", "-q", action="store_true", help="Show only essential information without colors and detailed formatting")
    parser.add_argument("--work-package-id", type=int, help="Override WORK_PACKAGE_ID from environment")
    ...
    try:
        ...
        if args.work_package_id:
            WORK_PACKAGE_ID_STR = str(args.work_package_id)
        ...
        if args.quiet:
            ...
            work_package = client.get_work_package(WORK_PACKAGE_ID)
            if work_package:
                print(f"Work Package {WORK_PACKAGE_ID}: {work_package.get('subject', 'No subject')}")
                print(f"Status: {work_package.get('_links', {}).get('status', {}).get('title', 'Unknown')}")
                print(f"Assignee: {work_package.get('_links', {}).get('assignee', {}).get('title', 'Unassigned')}")
                print(f"Created: {work_package.get('createdAt', 'Unknown')}")
                print(f"Updated: {work_package.get('updatedAt', 'Unknown')}")
            else:
                print(f"Failed to fetch work package {WORK_PACKAGE_ID}")
        else:
