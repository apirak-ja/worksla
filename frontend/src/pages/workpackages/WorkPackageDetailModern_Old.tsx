      ...
      <Container maxWidth="xl" sx={{ mt: -2, mb: 3 }}>
        <Zoom in timeout={400}>
          ...
          >
            <Box
              sx={{
                background: `linear-gradient(90deg, ${currentStatus.color} 0%, ${alpha(currentStatus.color, 0.7)} 100%)`,
                color: 'white',
                p: 2.5,
              }}
            >
              <Stack direction="row" alignItems="center" spacing={2}>
                ...
                <Chip
                  label={`${activities.filter((a: any) => a.details?.some((d: any) => d.property === 'Status')).length} การเปลี่ยนแปลง`}
                  size="small"
                  sx={{
                    bgcolor: 'rgba(255,255,255,0.25)',
                    color: 'white',
            ...
            <Box sx={{ p: 3 }}>
              <Stack direction="row" spacing={2} sx={{ overflowX: 'auto', pb: 1 }}>
                {activities
                  .filter((a: any) => a.details?.some((d: any) => d.property === 'Status'))
                  .map((activity: any, idx: number, filtered: any[]) => {
                    const statusDetail = activity.details.find((d: any) => d.property === 'Status');
                    return (
                      <Paper
                        key={idx}
                        elevation={2}
                        sx={{
                          ...
                          borderRadius: 2,
                          border: '2px solid',
                          borderColor: statusConfig[statusDetail.new_value]?.color || '#ccc',
                          position: 'relative',
                          background: `linear-gradient(135deg, ${alpha(statusConfig[statusDetail.new_value]?.color || '#ccc', 0.05)} 0%, ${alpha(statusConfig[statusDetail.new_value]?.color || '#ccc', 0.1)} 100%)`,
                        }}
                      >
                        <Stack spacing={1.5}>
                          <Stack direction="row" alignItems="center" spacing={1}>
                            {statusConfig[statusDetail.new_value]?.icon || <FiberManualRecord />}
                            <Typography variant="caption" fontWeight={700} color="text.secondary">
                              {statusDetail.old_value || 'เริ่มต้น'} → {statusDetail.new_value}
                            </Typography>
                          </Stack>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            📅 {format(new Date(activity.created_at), 'dd MMM yyyy HH:mm', { locale: th })}
                          </Typography>
                          {idx > 0 && (
                            <Chip
                              icon={<Schedule fontSize="small" />}
                              label={`⏱️ ${calculateDuration(activity.created_at, filtered[idx - 1].created_at)}`}
                              size="small"
                              sx={{
                                height: 26,
                                fontSize: '0.7rem',
                ...
                {activities.filter((a: any) => a.details?.some((d: any) => d.property === 'Status')).length === 0 && (
                  <Alert severity="info" sx={{ width: '100%' }}>
                    ยังไม่มีการเปลี่ยนสถานะ
                  </Alert>
                )}
              </Stack>
            </Box>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} lg={8}>
            ...
            <Card elevation={3} sx={{ borderRadius: 3 }}>
              ...
              >
                ...
                <Tab
                  icon={
                    <Badge badgeContent={activities.length} color="primary" max={999}>
                      <Timeline />
                    </Badge>
                  }
                  label="กิจกรรม"
              ...
              {/* Tab 1: Details - Empty because description and custom fields are already above */}
              <TabPanel value={tabValue} index={0}>
                <CardContent sx={{ p: 3 }}>
                  <Alert severity="info" icon={<Info />}>
                    📋 รายละเอียดและข้อมูลเพิ่มเติมแสดงอยู่ด้านบนแล้ว
                  </Alert>
                </CardContent>
              </TabPanel>
          ...
          <Grid item xs={12} lg={4}>
            <Stack spacing={3}>
              ...
              <Slide direction="left" in timeout={475}>
                <Card elevation={3} sx={{ borderRadius: 3 }}>
                  <CardContent sx={{ p: 3 }}>
                    <Typography variant="h6" fontWeight={700} mb={2}>
                      📍 สถานที่และกำหนดการ
                    ...
                    <Stack spacing={2}>
                      {wp.customField1 && (
                        <Box>
                          ...
                          <Typography variant="body2" fontWeight={700} pl={3.5}>
                            {wp.customField1}
                          </Typography>
                        </Box>
                      )}
              ...
              {/* Work Type & Priority Card */}
              <Slide direction="left" in timeout={490}>
                <Card elevation={3} sx={{ borderRadius: 3 }}>
                  <CardContent sx={{ p: 3 }}>
                    <Typography variant="h6" fontWeight={700} mb={2}>
